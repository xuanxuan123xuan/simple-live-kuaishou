...
class LiveRoomController extends PlayerController
  ...
  Duration? _onlineRefreshInterval;
  ...
  void _restartOnlineRefreshTimer() {
    ...
    final interval = showOfflineOverlay
        ? const Duration(seconds: 45)
        : const Duration(seconds: 10);
    _onlineRefreshInterval = interval;
    final refreshGeneration = _loadGeneration;
    final refreshSiteId = site.id;
    final refreshRoomId = roomId;
    _onlineRefreshTimer = Timer.periodic(interval, (_) async {
      ...
      try {
        ...
        if (roomDetail.resolvedLiveStatus != LiveStatusState.unknown) {
          detail.value = roomDetail;
          online.value = roomDetail.online;
        }
        _applyRoomLiveState(roomDetail);
        if (roomDetail.resolvedLiveStatus == LiveStatusState.live &&
            (!_hasActivePlaybackSession || playUrls.isEmpty)) {
          unawaited(_bootstrapPlaybackIfNeeded());
        }
        final desiredInterval = showOfflineOverlay
            ? const Duration(seconds: 45)
            : const Duration(seconds: 10);
        restartTimer = desiredInterval != _onlineRefreshInterval;
        if (restartTimer) {
  ...
  void _applyRoomLiveState(LiveRoomDetail roomDetail) {
    switch (roomDetail.resolvedLiveStatus) {
      ...
      case LiveStatusState.offline:
        offlineConfirmations.value += 1;
        if (offlineConfirmations.value >= 3 && !_hasActivePlaybackSession) {
          roomLiveState.value = LiveStatusState.offline;
  ...
  Future<bool> syncAutoPipOnLeave() async {
    ...
    if (!Platform.isAndroid ||
        ...
        (!liveStatus.value && !_hasActivePlaybackSession)) {
      if (Platform.isAndroid) {
  ...
  void onWSMessage(LiveMessage msg) {
    ...
    if (msg.type == LiveMessageType.chat) {
      ...
      if (isBackground && !_allowBackgroundPlayback) {
        return;
      }
  ...
  void loadData() async {
    ...
    try {
      ...
      // 刷新关注状态
      followed.value = DBService.instance.getFollowExist("${site.id}_$roomId");
      if (detail.value!.resolvedLiveStatus != LiveStatusState.unknown) {
        online.value = detail.value!.online;
      }
      _applyRoomLiveState(detail.value!);
      _restartSuperChatRefreshTimer();
      _restartOnlineRefreshTimer();
      unawaited(syncAutoPipOnLeave());
      if (detail.value!.resolvedLiveStatus == LiveStatusState.live) {
        unawaited(_bootstrapPlaybackIfNeeded());
      }
