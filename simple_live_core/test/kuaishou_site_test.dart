...
void main() {
  ...
  group('KuaishouSite.resolveLiveStatus', () {
    ...
    test('uses stream id evidence when the flag is stale or URLs are absent', () {
      expect(
    ...
    test('keeps missing live evidence unknown', () {
      expect(
        KuaishouSite.resolveLiveState({'author': {'id': 'anchor-id'}}),
        LiveStatusState.unknown,
      );
    });

    test('extracts URLs from reshaped playUrl payloads', () {
      expect(
        KuaishouSite.extractPlayableUrls({
          'streams': [
            {'address': 'https://example.com/live.m3u8'},
          ],
        }),
