...
class KuaishouSite extends LiveSite {
  ...
  static const _loginCookieNames = {
  ...
  void Function(String cookie, DateTime? expiresAt)? onCookieUpdated;

  String cookie = '';
  Map<String, String> cookieObj = {};

  static const _cookieRefreshInterval = Duration(minutes: 2);
  static const _didRegisterInterval = Duration(hours: 1);
  static const _roomDetailFallbackTtl = Duration(minutes: 3);

  DateTime? _lastCookieRefreshAt;
  DateTime? _lastDidRegisterAt;
  final Map<String, _KuaishouCachedRoomDetail> _roomDetailCache = {};
  ...
  Future<LiveRoomDetail> getRoomDetail({required String roomId}) async {
    ...
    final sessionCookie = _currentCookieHeader();
    if (sessionCookie.isNotEmpty) {
      cookieDetail = await _loadRoomDetail(
        url: url,
        roomId: roomId,
        headers: _headersWithCookie,
        danmakuCookie: sessionCookie,
        useSignedWebsocketInfo: true,
      );
      if (cookieDetail?.status == true) {
        return _rememberRoomDetail(roomId, cookieDetail!);
      }
    }
    ...
    final anonymousDetail = await _loadRoomDetail(
      ...
      headers: _headers,
      danmakuCookie: "",
      useSignedWebsocketInfo: false,
    );
    if (anonymousDetail?.status == true) {
      return _rememberRoomDetail(roomId, anonymousDetail!);
    }

    final resolvedDetail = cookieDetail ?? anonymousDetail;
    if (resolvedDetail != null) {
      return _rememberRoomDetail(roomId, resolvedDetail);
    }
    return _cachedRoomDetail(roomId) ?? _offlineDetail(roomId);
  }

  LiveRoomDetail _rememberRoomDetail(String roomId, LiveRoomDetail detail) {
    if (detail.status || detail.isRecord) {
      _roomDetailCache[roomId] = _KuaishouCachedRoomDetail(
        detail: detail,
        cachedAt: DateTime.now(),
      );
    } else {
      _roomDetailCache.remove(roomId);
    }
    return detail;
  }

  LiveRoomDetail? _cachedRoomDetail(String roomId) {
    final cached = _roomDetailCache[roomId];
    if (cached == null) {
      return null;
    }
    if (DateTime.now().difference(cached.cachedAt) > _roomDetailFallbackTtl) {
      _roomDetailCache.remove(roomId);
      return null;
    }
  ...
  Future<LiveRoomDetail?> _loadRoomDetail({
    ...
    required String danmakuCookie,
    required bool useSignedWebsocketInfo,
  }) async {
    try {
      ...
      return await _parseRoomDetail(
        resultText,
        roomId,
        danmakuCookie: danmakuCookie,
        useSignedWebsocketInfo: useSignedWebsocketInfo,
      );
    } catch (_) {
      return null;
    }
  ...
  Future<void> _getCookie(String url) async {
    ...
    final lastRefreshAt = _lastCookieRefreshAt;
    if (cookie.isNotEmpty &&
        lastRefreshAt != null &&
        now.difference(lastRefreshAt) < _cookieRefreshInterval) {
      return;
    }
    _lastCookieRefreshAt = now;
    try {
      final dio = Dio();
      ...
      DateTime? latestExpiresAt;
      for (final item in cookies) {
        cookieValues[item.name] = item.value;
        final expiresAt = item.expires;
        if (_loginCookieNames.contains(item.name) &&
            expiresAt != null &&
            (latestExpiresAt == null || expiresAt.isAfter(latestExpiresAt))) {
          latestExpiresAt = expiresAt;
        }
      }
      cookieObj = cookieValues;
      cookie = _formatCookieHeader(cookieValues);
      onCookieUpdated?.call(_currentCookieHeader(), latestExpiresAt);
    } catch (_) {
  ...
  Future<void> _registerDid() async {
    ...
    final now = DateTime.now();
    final lastRegisterAt = _lastDidRegisterAt;
    if (lastRegisterAt != null &&
        now.difference(lastRegisterAt) < _didRegisterInterval) {
      return;
    }
    _lastDidRegisterAt = now;
    try {
...
class _KuaishouCachedRoomDetail {
  final LiveRoomDetail detail;
  final DateTime cachedAt;

  const _KuaishouCachedRoomDetail({
