...
class KuaishouSite extends LiveSite {
  ...
  static LiveStatusState resolveLiveState(Map room) {
    final liveStreamValue = room["liveStream"];
    final liveStream = liveStreamValue is Map ? liveStreamValue : const {};
    final liveStreamId = liveStream["id"]?.toString().trim() ?? '';

    // A stream id is emitted by Kuaishou for an active stream even when the
    // accompanying playUrls object is delayed, empty, or reshaped.
  ...
  static bool resolveLiveStatus(Map room) {
    return resolveLiveState(room) == LiveStatusState.live;
  }
  ...
  static bool _containsPlayableUrl(dynamic value) {
    return extractPlayableUrls(value).isNotEmpty;
  }
  ...
  Future<LiveRoomDetail> getRoomDetail({required String roomId}) async {
    ...
    LiveRoomDetail? result;
    if (_currentCookieHeader().isNotEmpty) {
      result = await _loadRoomDetail(
        url: url,
        roomId: roomId,
    ...
    // 用实际游戏信息上报 DID 注册，避免硬编码触发风控。
    unawaited(_registerDid(
      gameId: result?.categoryId ?? '',
      gameName: result?.categoryName ?? '',
    ));

    if (result == null) {
      throw CoreError("快手直播间信息解析失败");
    }
    return result;
  }
  ...
  Future<LiveRoomDetail?> _parseRoomDetail(
    ...
    try {
      ...
      for (final item in playList) {
        ...
        first ??= item;
        if (resolveLiveState(item) == LiveStatusState.live) {
          first = item;
          break;
        }
      }
      ...
      final gameInfo = first["gameInfo"] is Map
          ...
          : const {};
      final liveState = resolveLiveState(first);
      final isLiving = liveState == LiveStatusState.live;
      final liveStreamId = liveStream["id"]?.toString() ?? '';
      var websocketUrls = <String>[];
      websocketUrls.addAll(_stringList(liveroom["websocketUrls"]));
      ...
      if (websocketUrls.isEmpty) {
        websocketUrls.addAll(_stringList(
      ...
      if (isLiving &&
        ...
        // 快手 API 有时在直播刚建立时 websocket 凭证尚未就绪，
        // 重试一次（带短暂延迟）以等待服务端生成完毕。
        for (var attempt = 0; attempt < 2; attempt++) {
          ...
          if (info.token.isNotEmpty) {
            danmakuToken = info.token;
          }
          if (info.websocketUrls.isNotEmpty) {
            websocketUrls = info.websocketUrls;
          }
          if (danmakuToken.isNotEmpty && websocketUrls.isNotEmpty) {
            break;
          }
        }
      }
  ...
  static List<String> _stringList(dynamic value) {
    ...
    if (value is Map) {
      return _stringList(value.values);
    }
    return const [];
  }
  ...
  Future<bool> getLiveStatus({required String roomId}) async {
    try {
      ...
      if (text == null) {
        ...
        return detail.resolvedLiveStatus == LiveStatusState.live;
      }
      ...
      return resolveLiveStatus(playList.first as Map);
    } catch (_) {
      try {
        final detail = await getRoomDetail(roomId: roomId);
        return detail.resolvedLiveStatus == LiveStatusState.live;
  ...
  Future<List<LivePlayQuality>> getPlayQualites({
    ...
    try {
      ...
      final h264 = data is Map ? data["h264"] : null;
      final adaptationSet = h264 is Map ? h264["adaptationSet"] : null;
      final qualityList =
          adaptationSet is Map ? adaptationSet["representation"] : null;

      if (qualityList is Iterable) {
        for (final quality in qualityList) {
          if (quality is! Map) {
            continue;
          }
          final urls = extractPlayableUrls(quality["url"]);
          if (urls.isEmpty) {
            continue;
          }
          qualities.add(
      ...
      if (qualities.isEmpty) {
        final urls = extractPlayableUrls(data);
        if (urls.isNotEmpty) {
          qualities.add(
            LivePlayQuality(quality: '默认', sort: 0, data: urls),
          );
        }
      }
    } catch (_) {}
  ...
  Future<void> _getCookie(String url) async {
    try {
      ...
      await dio
          .get(url, options: Options(headers: _headersWithCookie))
  ...
  /// [gameId] 和 [gameName] 可选，用于上报用户当前浏览的直播间分类；
  /// 留空则使用通用占位符，避免因硬编码特定游戏名触发风控。
  Future<void> _registerDid({String gameId = '', String gameName = ''}) async {
    ...
    try {
      await HttpClient.instance.postJson(
        ...
        header: _headers,
        data: _buildMisc2Data(did, gameId: gameId, gameName: gameName),
      );
    } catch (_) {}
  }
  ...
  Map<String, dynamic> _buildMisc2Data(
    ...
    final safeGameId = gameId.isNotEmpty ? gameId : '0';
    final safeGameName = gameName.isNotEmpty ? gameName : '未知';
    return {
      'common': {
          ...
          'event_package': {
            'task_event': {
              ...
              'url_package': {
                ...
                'page_type': 2,
                'params': '{"game_id":$safeGameId,"game_name":"$safeGameName"}',
              },
              'element_package': {},
            },
          },
